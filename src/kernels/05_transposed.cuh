...
__global__ void sgemm_transposed_kernel(int M, int N, int K, float alpha,
  ...
  const int A_smem_col = threadIdx.x % (BK / 4);
  const int A_smem_row = threadIdx.x / (BK / 4);
  const int B_smem_col = threadIdx.x % (BN / 4);
  ...
  for (int k_tile = 0; k_tile < K; k_tile += BK) {
    ...
    for (int A_row_offset = 0; A_row_offset < BM;
      ...
      float4 tmp = reinterpret_cast<float4 *>(
          &A[(A_smem_row + A_row_offset) * K + A_smem_col * 4])[0];
      A_tile[(A_smem_col * 4 + 0) * BM + (A_smem_row + A_row_offset)] = tmp.x;
      A_tile[(A_smem_col * 4 + 1) * BM + (A_smem_row + A_row_offset)] = tmp.y;
      A_tile[(A_smem_col * 4 + 2) * BM + (A_smem_row + A_row_offset)] = tmp.z;
      A_tile[(A_smem_col * 4 + 3) * BM + (A_smem_row + A_row_offset)] = tmp.w;
    }
    ...
    for (int B_row_offset = 0; B_row_offset < BK;
      ...
      reinterpret_cast<float4 *>(
          &B_tile[(B_smem_row + B_row_offset) * BN + B_smem_col * 4])[0] =
          reinterpret_cast<float4 *>(
              &B[(B_smem_row + B_row_offset) * N + B_smem_col * 4])[0];
    }
    ...
    for (int k = 0; k < BK; ++k) {
      ...
      for (int i = 0; i < (TM / 4); i++) {
        ...
        reinterpret_cast<float4 *>(&A_reg[i * 4])[0] =
            reinterpret_cast<float4 *>(
                &A_tile[k * BM + C_inner_row + i * 4])[0];
      }
      ...
      for (int j = 0; j < (TN / 4); j++) {
        // This explicit vectorization does not improve performance
        // which means the loads were already vectorized
        reinterpret_cast<float4 *>(&B_reg[j * 4])[0] =
            reinterpret_cast<float4 *>(
                &B_tile[k * BN + C_inner_col + j * 4])[0];
      }
  ...
  for (int i = 0; i < TM; i++) {
    for (int j = 0; j < (TN / 4); j++) {
      ...
      float4 tmp = reinterpret_cast<float4 *>(
          &C[(C_inner_row + i) * N + (C_inner_col + j * 4)])[0];
      tmp.x = alpha * sums[i * TN + j * 4 + 0] + beta * tmp.x;
      tmp.y = alpha * sums[i * TN + j * 4 + 1] + beta * tmp.y;
      tmp.z = alpha * sums[i * TN + j * 4 + 2] + beta * tmp.z;
      tmp.w = alpha * sums[i * TN + j * 4 + 3] + beta * tmp.w;
      reinterpret_cast<float4 *>(
          &C[(C_inner_row + i) * N + (C_inner_col + j * 4)])[0] = tmp;
    }
  }
}
