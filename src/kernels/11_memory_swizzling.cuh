...
__global__ void memory_swizzling_kernel(int M, int N, int K, float alpha,
  ...
  constexpr unsigned int a_shmem_row_stride = (num_threads * 8) / BK;
  constexpr unsigned int a_shmem_iters = BM / a_shmem_row_stride;

  constexpr unsigned int b_shmem_row_stride = (num_threads * 8) / BN;
  constexpr unsigned int b_shmem_iters = BK / b_shmem_row_stride;
  ...
  constexpr unsigned int a_swizzle_bits = int_log2(BK / 8);
  constexpr unsigned int a_swizzle_mask = 0b111 << (3 + a_swizzle_bits);
  constexpr unsigned int a_swizzle_mask_bytes = a_swizzle_mask << 1;

  constexpr unsigned int b_swizzle_bits = int_log2(BN / 8);
  constexpr unsigned int b_swizzle_mask = 0b111 << (3 + b_swizzle_bits);
  constexpr unsigned int b_swizzle_mask_bytes = b_swizzle_mask << 1;

  for (int block_k = 0; block_k < K; block_k += BK) {
    unsigned int a_shmem_row = (threadIdx.x * 8) / BK;
    const unsigned int a_shmem_col = (threadIdx.x * 8) % BK;

    // Load swizzled A tile into shared memory
    for (unsigned int i = 0; i < a_shmem_iters; i++) {
      const unsigned src_idx = a_shmem_row * K + (block_k + a_shmem_col);
      unsigned dst_idx = a_shmem_row * BK + a_shmem_col;
      dst_idx = dst_idx ^ ((dst_idx & a_swizzle_mask) >> a_swizzle_bits);
      reinterpret_cast<float4 *>(&a_shmem[dst_idx])[0] =
          reinterpret_cast<const float4 *>(&d_A[src_idx])[0];
      a_shmem_row += a_shmem_row_stride;
    }

    unsigned int b_shmem_row = (threadIdx.x * 8) / BN;
    const unsigned int b_shmem_col = (threadIdx.x * 8) % BN;

    // Load swizzled B tile into shared memory
    for (unsigned int i = 0; i < b_shmem_iters; i++) {
      const unsigned src_idx = (block_k + b_shmem_row) * N + b_shmem_col;
      unsigned dst_idx = b_shmem_row * BN + b_shmem_col;
      dst_idx = dst_idx ^ ((dst_idx & b_swizzle_mask) >> b_swizzle_bits);
      reinterpret_cast<float4 *>(&b_shmem[dst_idx])[0] =
          reinterpret_cast<const float4 *>(&d_B[src_idx])[0];
    ...
    for (int warp_k = 0; warp_k < BK; warp_k += WK) {
      half *A_warp_tile = &a_shmem[warptile_row * BK + warp_k];
      ...
      for (int m_tile = 0; m_tile < TILES_M; m_tile++) {
        for (int k_frag = 0; k_frag < TILES_K; k_frag++) {
          const unsigned int mma_tile_byte_offset =
              ((m_tile * MMA_M * BK) + (k_frag * MMA_K)) * sizeof(half);
          const unsigned int thread_byte_offset =
              (threadIdx.x % MMA_M) * BK * sizeof(half);
          unsigned int thread_offset_bytes = A_warp_tile_byte_offset +
                                             mma_tile_byte_offset +
                                             thread_byte_offset;
          thread_offset_bytes =
              thread_offset_bytes ^
              ((thread_offset_bytes & a_swizzle_mask_bytes) >> a_swizzle_bits);
      ...
      half *B_warp_tile = &b_shmem[warp_k * BN + warptile_col];
      ...
      for (int k_frag = 0; k_frag < TILES_K; k_frag++) {
        for (int n_tile = 0; n_tile < TILES_N; n_tile++) {
          const unsigned int mma_tile_byte_offset =
              ((k_frag * MMA_K * BN) + (n_tile * MMA_N)) * sizeof(half);
          const unsigned int thread_byte_offset =
              (threadIdx.x % MMA_K) * BN * sizeof(half);
          unsigned int thread_offset_bytes = B_warp_tile_byte_offset +
                                             mma_tile_byte_offset +
                                             thread_byte_offset;
          thread_offset_bytes =
              thread_offset_bytes ^
              ((thread_offset_bytes & b_swizzle_mask_bytes) >> b_swizzle_bits);
